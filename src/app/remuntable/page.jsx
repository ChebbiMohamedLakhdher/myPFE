  ...
  return (
    <TableContainer component={Paper} className="table">
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        ...
        <TableBody>
          {Array.isArray(remforms) &&
            remforms.map((remform, index) => (
              <TableRow key={index}>
                <TableCell className="tableCell">{remform.title}</TableCell>{" "}
                {/* Assuming title is the property for user's name */}
                <TableCell className="tableCell">
                  {remform.persons.join(", ")}
                </TableCell>{" "}
                {/* Assuming persons is an array, join it with comma */}
                <TableCell className="tableCell">
